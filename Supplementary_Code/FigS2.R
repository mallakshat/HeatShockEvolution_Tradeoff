#S2 - evolved vs ancestor in evolution regime
...
plot_concentration <- function(env) {
  ggplot() +
    ...
    ggtitle(env)+xlab(NULL)+ylab(NULL)+ylim(0,0.18)+xlim(0,35)
    #guides(color = guide_legend(override.aes = list(size = 7)))+xlim(0,40) 
}

tiff("Supplementary/HS growth curves/J1_HS.tiff", width = 5, height = 5, units = "in", res = 300)
