data = read.csv("Supplementary_Data/QurveAnalysis_Evolved_30C.csv")
...
tiff("Figures/Evolved_Lag_30C.tiff", width = 5, height = 5, units = "in", res = 300)
ggplot(summary_df, aes(x = Isolate, y = mean_lag)) +
  geom_rect(aes(xmin = -Inf, xmax = Inf, ymin = 4.23, ymax = 4.81),fill = "gray80", alpha = 0.5) +
  geom_hline(yintercept = 4.523, linetype = "dashed", color = "black", linewidth = 0.6) +
  ...
  geom_text(aes(label = sig, y = mean_lag + se_lag + 0.6),size = 8)+
...
tiff("Figures/Evolved_GrowthRate_30C.tiff", width = 5, height = 5, units = "in", res = 300)
ggplot(summary_df, aes(x = Isolate, y = mean_gr_rate)) +
  ...
  #geom_point(size = 5,color="black") + geom_point(size=3,color="gray70")+
  geom_text(aes(label = sig_gr, y = mean_gr_rate + se_gr_rate + 0.01),size = 8)+
  theme_classic()+theme(text = element_text(size = 26),axis.text.x = element_text(size = 12),legend.position = "none")+
  ylim(0,0.25)+
